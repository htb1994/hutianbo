...
function localizeTemplateBody(body, context) {
  ...
  return localizeTemplateWording(String(body || ""), {
    ...
    .replaceAll(`${targetRegion || context.city}小学、初中、高中`, `${targetRegion || context.city}${stageText}`)
    .replaceAll("小学、初中、高中", stageText)
    .replaceAll("小学重点讲习惯和兴趣，初中重点讲漏洞和错题，高中重点讲规划和效率。", stageFocusSentence(stageText))
    .replaceAll("小学孩子看兴趣和习惯，初中孩子看知识点是否听懂，高中孩子看能否跟上节奏。", stageChildFocusSentence(stageText))
    .replaceAll("不是所有孩子都适合直接培优。小学看兴趣和理解力，初中看基础漏洞，高中看时间规划和效率。", `不是所有孩子都适合直接培优。${stageText}孩子先看基础掌握和学习状态，适合的才加难度，不适合的先补基础。`)
    .replaceAll("对应年级的组合品", `${stageText}组合品`)
    .replaceAll("同步学、专项突破、学情报告、组合品", productText);
}

function stageFocusSentence(stageText) {
  if (stageText.includes("初中") && !stageText.includes("小学") && !stageText.includes("高中")) {
    return "初中重点讲知识漏洞、错题复盘和开学衔接。";
  }
  if (stageText.includes("小学") && !stageText.includes("初中") && !stageText.includes("高中")) {
    return "小学重点讲学习习惯、兴趣保护和每日完成感。";
  }
  if (stageText.includes("高中") && !stageText.includes("小学") && !stageText.includes("初中")) {
